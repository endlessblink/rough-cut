...
function censorRegionsFor(frame){const list=frame&&frame.censorRegions;return Array.isArray(list)?list.filter((region)=>region&&region.rect):[]}
// The GPU path composites into its own canvas, so a 2D censor pass drawn on ctx
// would never reach it. Rather than write a second censor implementation as a
// shader, frames carrying a censor take the Canvas2D path. Slower on those frames,
// and correct — an uncensored export is not a performance trade worth making.
function useGpuFrame(frame){return !!gpu&&frame&&frame.screen&&censorRegionsFor(frame).length===0}
function censorSourceRectFor(region,vw,vh){const rect=region&&region.rect;if(!rect)return null;const x1=Math.max(0,Math.min(1,Number(rect.x)));const y1=Math.max(0,Math.min(1,Number(rect.y)));const x2=Math.max(0,Math.min(1,Number(rect.x)+Number(rect.w)));const y2=Math.max(0,Math.min(1,Number(rect.y)+Number(rect.h)));const left=Math.min(x1,x2)*vw;const top=Math.min(y1,y2)*vh;const width=Math.abs(x2-x1)*vw;const height=Math.abs(y2-y1)*vh;if(!(width>0)||!(height>0))return null;return {x:left,y:top,w:width,h:height}}
function mapCensorToDest(censorRect,sourceRect,destRect){if(!censorRect||!sourceRect||!destRect)return null;const sw=sourceRect.width;const sh=sourceRect.height;if(!(sw>0)||!(sh>0)||!(destRect.width>0)||!(destRect.height>0))return null;const left=Math.max(censorRect.x,sourceRect.x);const top=Math.max(censorRect.y,sourceRect.y);const right=Math.min(censorRect.x+censorRect.w,sourceRect.x+sw);const bottom=Math.min(censorRect.y+censorRect.h,sourceRect.y+sh);if(right<=left||bottom<=top)return null;const scaleX=destRect.width/sw;const scaleY=destRect.height/sh;return {x:destRect.x+(left-sourceRect.x)*scaleX,y:destRect.y+(top-sourceRect.y)*scaleY,w:(right-left)*scaleX,h:(bottom-top)*scaleY}}
// Mirrors censor-overlay.ts: mosaic sampled from the video into a small offscreen
// buffer, optional soften applied while drawing that buffer back up so the blur
// only ever sees already-destroyed pixels, and solid fill as the fail-closed path.
function drawCensorRegionsExport(frame,screen){const regions=censorRegionsFor(frame);if(regions.length===0)return 0;const video=videoFor(frame.screen);const layer=frame.screen||{};const sourceSize=layer.sourceSize||{};const vw=(video&&video.videoWidth)||sourceSize.width||screen.width;const vh=(video&&video.videoHeight)||sourceSize.height||screen.height;const sourceRect=video?screenSourceRect(video,layer,screen):null;if(!sourceRect)return 0;let painted=0;for(const region of regions){const censorSource=censorSourceRectFor(region,vw,vh);const dest=mapCensorToDest(censorSource,sourceRect,{x:screen.x,y:screen.y,width:screen.width,height:screen.height});if(!dest)continue;ctx.save();ctx.beginPath();ctx.rect(dest.x,dest.y,dest.w,dest.h);ctx.clip();let destroyed=false;if(region.mode==='pixelate'&&video){const blockSize=Number.isFinite(region.blockSize)&&region.blockSize>0?region.blockSize:24;const cols=Math.min(512,Math.max(1,Math.round(censorSource.w/blockSize)));const rows=Math.min(512,Math.max(1,Math.round(censorSource.h/blockSize)));const buffer=censorBuffer(cols,rows);if(buffer){buffer.ctx.clearRect(0,0,cols,rows);buffer.ctx.drawImage(video,censorSource.x,censorSource.y,censorSource.w,censorSource.h,0,0,cols,rows);const scale=dest.w/Math.max(1,censorSource.w);const soften=region.soften!==false?Math.min(24,Math.max(0.5,blockSize*0.35*(scale>0?scale:1))):0;if(soften>0){ctx.filter='blur('+soften.toFixed(2)+'px)';ctx.imageSmoothingEnabled=true}else ctx.imageSmoothingEnabled=false;ctx.drawImage(buffer.canvas,0,0,cols,rows,dest.x,dest.y,dest.w,dest.h);ctx.filter='none';destroyed=true}}
if(!destroyed){const fill=typeof region.fillColor==='string'&&/^#[0-9a-fA-F]{6}$/.test(region.fillColor)?region.fillColor:'#0b0f14';ctx.fillStyle=fill;ctx.fillRect(dest.x,dest.y,dest.w,dest.h)}
ctx.filter='none';ctx.imageSmoothingEnabled=true;ctx.restore();painted+=1}
return painted}
let censorBufferCanvas=null;
function censorBuffer(width,height){if(!censorBufferCanvas)censorBufferCanvas=document.createElement('canvas');censorBufferCanvas.width=width;censorBufferCanvas.height=height;const bufferCtx=censorBufferCanvas.getContext('2d');if(!bufferCtx)return null;return {canvas:censorBufferCanvas,ctx:bufferCtx}}
...
window.__roughCutRenderHeadlessFrame=async(frame,index)=>{
  ...
  // Censors go on after the screen and before the cursor, matching the preview so
  // the cursor stays visible over a censored area in both.
  const censoredCount=screen?drawCensorRegionsExport(frame,screen):0;
