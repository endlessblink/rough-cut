...
async function renderHeadlessFrames({ compositionPlan, outputPath, electronRuntime = null, createRenderWindow = null } = {}) {
  ...
  try {
    ...
    for (const [index, frame] of frames.entries()) {
      ...
      const png = nativeImageToPng(image);
...
function drawGpuVideo(video,rect,source){if(!gpu)return false;try{gl.bindTexture(gl.TEXTURE_2D,gpu.texture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,video);drawGpuQuad(rect,{texture:true,source,textureWidth:video.videoWidth||rect.width,textureHeight:video.videoHeight||rect.height});return true}catch{return false}}
...
function sourcePoint(frame,screen,source){const size=frame.screen&&frame.screen.sourceSize;if(!source||!size||!Number.isFinite(size.width)||!Number.isFinite(size.height))return null;const normalizedX=Math.abs(source.x)>1?source.x/size.width:source.x;const normalizedY=Math.abs(source.y)>1?source.y/size.height:source.y;return {x:screen.x+normalizedX*screen.width,y:screen.y+normalizedY*screen.height}}
...
window.__roughCutRenderHeadlessFrame=async(frame,index)=>{
  ...
  let drewScreen=null;
  let usedGpu=false;
  if(screen&&useGpu){drewScreen=await drawGpuScreenLayer(frame.screen,screen);if(drewScreen)usedGpu=true;else{clearGpu('#000000');drewBackgroundImage=await drawBackground(frame);fallbackScreen(screen)}}
  ...
  let drewCamera=null;
  if(camera&&useGpu&&usedGpu){drewCamera=await drawGpuCameraLayer(frame.camera,camera);if(!drewCamera){usedGpu=false;clearGpu('#000000');drewBackgroundImage=await drawBackground(frame);fallbackScreen(screen);fallbackCamera(camera)}}
  ...
  return {ok:true,index,frameIndex:frame&&frame.frameIndex,timelineGap,rendererKind:usedGpu?'webgl':'canvas2d',drewScreen,drewCamera,drewBackgroundImage,cursorPoint:point?{x:Math.round(point.x*100)/100,y:Math.round(point.y*100)/100}:null,clickPoint:clicked?{x:Math.round(clicked.x*100)/100,y:Math.round(clicked.y*100)/100}:null};
};
</script>`;
}
