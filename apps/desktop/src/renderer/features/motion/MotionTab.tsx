            ...
            {templates.filter((t) => t.category === category).map((t) => (
              <button
                ...
                style={{
                  padding: '10px 12px',
                  borderRadius: 8,
                  background: selectedId === t.id ? 'rgba(37, 99, 235, 0.15)' : 'rgba(255,255,255,0.02)',
                  border: selectedId === t.id ? '1px solid rgba(37, 99, 235, 0.3)' : '1px solid rgba(255,255,255,0.04)',
                  cursor: 'pointer',
                  textAlign: 'left',
                  fontFamily: 'inherit',
                  display: 'flex',
              ...
              >
                <span style={{ fontSize: 13, fontWeight: 600, color: selectedId === t.id ? '#fff' : 'rgba(255,255,255,0.7)' }}>
                  {t.name}
                </span>
...
export function MotionTab({ activeTab, onTabChange }: MotionTabProps) {
  ...
  const [currentProps, setCurrentProps] = useState<Record<string, unknown>>(template.defaultProps);

  // When template changes, reset props
  const handleSelectTemplate = useCallback((id: string) => {
    setSelectedTemplateId(id);
    const t = TEMPLATE_REGISTRY.find((t) => t.id === id)!;
    setCurrentProps({ ...t.defaultProps });
  }, []);
  ...
  return (
    <div data-testid="motion-tab-root" style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0d0d0d', color: '#e8e8e8', overflow: 'hidden' }}>
      <AppHeader
        activeTab={activeTab}
      ...
      <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'row', minHeight: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>
            Templates
          </div>
          <TemplateBrowser
