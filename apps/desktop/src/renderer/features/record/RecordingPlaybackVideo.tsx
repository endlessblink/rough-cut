...
export function RecordingPlaybackVideo({
  ...
  useEffect(() => {
    ...
    console.info('[RecordingPlaybackVideo] Cursor overlay ready:', data.frameCount, 'frames,', cursorEvents.length, 'events');
    setCursorData(data);
  ...
  // Track transport state for reticle visibility (only show when paused + in marker range)
  const [transportFlags, setTransportFlags] = useState<{ isPlaying: boolean; playheadFrame: number }>(() => {
    const s = transportStore.getState();
    return { isPlaying: s.isPlaying, playheadFrame: s.playheadFrame };
  });
  useEffect(() => {
    const unsub = transportStore.subscribe((s) => {
      setTransportFlags({ isPlaying: s.isPlaying, playheadFrame: s.playheadFrame });
    });
    return unsub;
  }, []);
  ...
  const zt = zoomMarkers.length > 0
    ? getZoomTransformAtFrame(transportFlags.playheadFrame - clipTimelineIn, zoomMarkers)
