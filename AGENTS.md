...
- Electron runtime tests may fail inside Codex managed sandbox with `sandbox_host_linux.cc(41)` / `NoNewPrivs: 1` / `Seccomp: 2`. First prefer the persistent host bridge: ask the user to run `scripts/task246-host-runner.sh` once in a normal host terminal and leave it open, then trigger runs from Codex with `touch /tmp/rough-cut-task246-host-runner.request` and inspect `/tmp/rough-cut-task246-host-runner.status.json` plus `/tmp/rough-cut-task246-host-runner.log`. Only if the agent must own Electron directly, launch a fresh Codex session through `scripts/launch-codex-electron-test.sh` or the `codex-electron-test` shell alias; the profile is `~/.codex/electron-test.config.toml`.
