...
test('GPU-C experimental headless renderer seam stays opt-in and fallback-backed', () => {
  ...
  assert.match(headlessExportRendererSource, /if\(drewScreen\)usedGpu=true;else\{clearGpu\('#000000'\);drewBackgroundImage=await drawBackground\(frame\);fallbackScreen\(screen\)\}/);
  ...
  assert.match(headlessExportRendererSource, /if\(screen&&useGpu\)\{drewScreen=await drawGpuScreenLayer\(frame\.screen,screen\)/);
  assert.match(headlessExportRendererSource, /if\(camera&&useGpu&&usedGpu\)\{drewCamera=await drawGpuCameraLayer\(frame\.camera,camera\)/);
  assert.match(headlessExportRendererSource, /if\(!drewCamera\)\{usedGpu=false;clearGpu\('#000000'\);drewBackgroundImage=await drawBackground\(frame\);fallbackScreen\(screen\);fallbackCamera\(camera\)\}/);
  ...
  assert.match(experimentalHeadlessRuntimeExportSmokeSource, /webglFrameCount < 1/);
  assert.match(experimentalHeadlessRuntimeExportSmokeSource, /Experimental runtime renderer did not produce any WebGL frames/);
