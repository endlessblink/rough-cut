...
const compositionLayoutTestSource = readFileSync(join(root, 'packages/frame-resolver/src/composition-layout.test.ts'), 'utf8');
...
test('GPU-C experimental headless renderer seam stays opt-in and fallback-backed', () => {
  ...
  assert.match(exportServiceSource, /resolveHeadlessScreenLayout,/);
  assert.match(exportServiceSource, /resolveHeadlessCameraLayout,/);
  assert.match(exportServiceSource, /normalizeCompositionPresentationStyle as normalizePresentationStyle/);
  assert.doesNotMatch(exportServiceSource, /function resolveHeadlessScreenLayout\(frame\)/);
  assert.doesNotMatch(exportServiceSource, /function resolveHeadlessCameraLayout\(frame\)/);
  assert.match(compositionLayoutSource, /export function resolveHeadlessScreenLayout\(frame: CompositionLayoutFrame\)/);
  assert.match(compositionLayoutSource, /export function resolveHeadlessCameraLayout\(frame: CompositionLayoutFrame\)/);
  assert.match(compositionLayoutSource, /export function normalizeCompositionPresentationStyle/);
  assert.match(compositionLayoutTestSource, /resolves default headless screen layout from background padding and source aspect/);
  assert.match(compositionLayoutTestSource, /resolves manual headless screen layout with crop viewport aspect/);
  assert.match(compositionLayoutTestSource, /resolves presentation camera layout and style metadata/);
  assert.match(compositionLayoutTestSource, /resolves manual circular camera frames through square constrained bounds/);
  ...
  assert.match(compositionLayoutSource, /shape: presentation\.shape/);
  assert.match(compositionLayoutSource, /shadowEnabled: presentation\.shadowEnabled !== false/);
  assert.match(compositionLayoutSource, /shadowBlur: Number\.isFinite\(presentation\.shadowBlur\) \? presentation\.shadowBlur : 24/);
  assert.match(compositionLayoutSource, /shadowOpacity: Number\.isFinite\(presentation\.shadowOpacity\) \? presentation\.shadowOpacity : 0\.45/);
