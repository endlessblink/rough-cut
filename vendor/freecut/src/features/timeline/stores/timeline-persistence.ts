...
import { persistRoughCutProject } from '@/infrastructure/storage/rough-cut-host'
import {
...
export async function saveTimeline(projectId: string): Promise<void> {
  ...
  try {
    ...
    if (project.roughCutPath) {
      await persistRoughCutProject({ ...project, timeline: sanitizedTimeline })
    }
