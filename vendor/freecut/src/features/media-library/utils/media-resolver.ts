...
export async function resolveMediaUrl(mediaId: string): Promise<string> {
  ...
  const requestPromise = (async () => {
    ...
    try {
      ...
      // Rough Cut owns the source file and exposes a range-capable local URL;
      // keep playback streaming instead of buffering an entire recording into
      // a blob before the FreeCut preview can render.
      if (media.roughCutUrl) {
        useMediaLibraryStore.getState().markMediaHealthy(mediaId)
        return media.roughCutUrl
      }
