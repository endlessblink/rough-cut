...
class MediaLibraryService {
  ...
  async getMediaFile(idOrMedia: string | MediaMetadata): Promise<Blob | null> {
    ...
    // Embedded Rough Cut projects keep their source files in Rough Cut's
    // native recordings directory instead of copying them into FreeCut.
    if (media.roughCutUrl) {
      try {
        const response = await fetch(media.roughCutUrl, { cache: 'no-store' })
        if (!response.ok) throw new Error(`Host media request failed (${response.status})`)
        return await response.blob()
      } catch (error) {
        logger.warn(`Rough Cut media ${id} could not be read:`, error)
        return null
      }
    }
